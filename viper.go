...
func ReadInConfig(configByte []byte) error { return v.ReadInConfig(configByte) }
func (v *Viper) ReadInConfig(configByte []byte) error {
	...
	if len(configByte) == 0 {
		...
		err = v.unmarshalReader(bytes.NewReader(configByte), config)
	}
	if err != nil {
		return err
	}
